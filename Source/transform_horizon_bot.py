...
@author: syang34
"""
# Compared to flat_horizon_v3.1, this version creates triangular meshes using 
# cell vertex instead of cell center
import pyvista as pv
import numpy as np

#
xyz = np.loadtxt('Input/bot_surface_point.txt')
...
cellsize = round(np.sum((xyz[0, 2:4] - xyz[1, 2:4]) ** 2) ** 0.5, 0)
left_cell,cell_neighbor=rh.get_leftmost_cell(pointdata=xyz,cellsize=cellsize)
row_face_coll=[]
for i in range(len(left_cell)-1):
    row1=rh.get_row_cell(cell_adj=cell_neighbor,leftcell=left_cell[i],cellsize=cellsize)
    row2=rh.get_row_cell(cell_adj=cell_neighbor,leftcell=left_cell[i+1],cellsize=cellsize)
...
# Output
np.savetxt('Output/restored_bot_point_data.txt',np.array(pt_data), fmt = '%d %f %f %f',
